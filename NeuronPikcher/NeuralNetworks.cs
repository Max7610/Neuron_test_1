...
namespace NeuronPikcher
{
    class NeuralNetworks
    {
        ...
        public Neuron FeedForward( double[] inputSignal)
        {
        ...
        public double Learn(double[] expected,double[,] inputs, int epoch)
        {
            ...
            for (int i = 0; i < epoch; i++)//такты отладки
            {
                ...
                for (int j = 0; j < expected.Length; j++)//перебор массива значений для отладки значений
                {
                    ...
                    errorEpocha += e;// запуск одного такта отладки
                    error += e;
                    n++;
                    if ((j * 100) % expected.Length == 0)
                    {
                        Console.WriteLine($"обучение, эпоха {i}  {(j * 100) / expected.Length}% ошибка  {error / n }    ");
                        error = 0.0;
                        n = 0;
